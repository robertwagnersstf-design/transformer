...
public:
    struct AttentionCache {
        ...
        : input(rows, cols), d_input(rows, cols), q(rows, cols), k(rows, cols), v(rows, cols), score(rows, rows), output(rows, cols), d_output(rows, cols), score_heads(), d_out_heads(), output_w0(rows, cols) {}
    };
    ...
    Matrix& forward_mha(Matrix & input);
